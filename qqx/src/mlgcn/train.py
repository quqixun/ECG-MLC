...
from models import GraphResNeXt
...
class ECGTrainer(object):
    ...
    def __build_model(self, **params):
        self.model = GraphResNeXt(
            kss=[15, 15, 11, 11, 7], layers=[3, 3, 3, 3, 3],
            groups=8, dropout=0.5, hrv_feats=2212,
            graph_pkl='./graph.pkl'
        )
        if self.cuda:
            self.model.cuda()
    ...
    def __build_scheduler(self):
        ...
        #     self.optimizer, milestones=[21, 81], gamma=0.333)
        return

    def run(self, signal_dir, hrv_dir, trainset, validset, model_dir):
        print('=' * 70 + '\n' + 'TRAINING MODEL\n' + '-' * 70 + '\n')
        model_path = os.path.join(model_dir, 'model.pth')
        thresh_path = os.path.join(model_dir, 'threshold.npy')

        dataloader = {
            'train': ECGLoader(trainset, signal_dir, hrv_dir, self.batch_size, True).build(),
            'valid': ECGLoader(validset, signal_dir, hrv_dir, self.batch_size, False).build()
        }
        ...
        for epoch in range(self.n_epochs):
            ...
            for phase in ['train', 'valid']:
                ...
                for ith_batch, data in enumerate(dataloader[phase]):
                    ecg, hrv, label = [d.cuda() for d in data] if self.cuda else data

                    pred = self.model(ecg, hrv)
...
class ECGTrain(object):
    ...
    def __train(self, signal_dir, hrv_dir, trainset, validset, model_dir):
        trainer = ECGTrainer(**self.params)
        trainer.run(signal_dir, hrv_dir, trainset, validset, model_dir)
        return

    def run(self, signal_dir, hrv_dir, labels_csv, models_dir):
        ...
        for i in range(self.cv):
            ...
            self.__train(signal_dir, hrv_dir, trainset, validset, model_dir)
