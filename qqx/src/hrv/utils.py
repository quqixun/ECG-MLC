...
class FBetaLoss(nn.Module):
    ...
    def forward(self, inputs, targets):
        ...
        def f(i, t):
            tp = (i * t).sum(dim=1)
            precision = tp.div(i.sum(dim=1).add(self.eps))
            recall = tp.div(t.sum(dim=1).add(self.eps))
        ...
        fbeta_1 = f(inputs, targets)
        fbeta_0 = f(1 - inputs, 1 - targets)

        return 2 - (fbeta_1 + fbeta_0)
